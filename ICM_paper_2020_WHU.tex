...
\begin{itemize}
	...
	\item \textbf{用户在评论前可以看到之前用户的评价星级，但不能看到很久以前的用户的评论星级。即只有前一个时间段的平均星级才对下一个时间段的评论产生影响。}.
	
	\item \textbf{评论情感倾向与星级相符。}.
	
	\item \textbf{一个时间段内的评论数量与情感满足正态分布。}
	
	\item \textbf{用户给出的评价和星级只与当前产品相关。}
...
在我们的建模中，因为有且只有特定的星级会引起用户更多的评论，显然只有星级一个自变量，所以我们采用单变量方差分析\cite{stoline1981status}，我们的Model to relationship between specific star ratings and more reviews based on One-Way ANOVA \cite{One-Way ANOVA} 建立如下\\
\begin{enumerate}
	\item 我们先对数据进行筛选，选择每类产品最具代表性的样本:
	\begin{itemize}
