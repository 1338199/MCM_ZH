...
我们可以从产品的星级和评论推断出产品的声誉，而一个产品的声誉越好往往意味着产品的销售前景越好，在这里，我们不能单纯地用星级或评价衡量一个产品的声誉，因为生活中往往存在着“五星好评”和“一星黑粉”的情况。因此我们综合运用星级和评论构造出声誉的模型，通过比对产品的声誉，阳光公司可以针对性提升声誉高的产品的备货量和宣传力度。
...
通过建模，并对2015年7月-8月评论数大于25的pacifer样本进行计算，我们发现，在7-8月，样本xxxx的声誉更高，销售前景最大；样本xxxx的声誉最低，销售前景最差。
...
通过对评级和评论建模可以得出产品的综合声誉，而更高的声誉往往意味着更好的销售前景，可以帮助阳光公司作出更好的决策。建模过程如下：

假定产品的星级为$R_{\alpha}$, 评论量化之后为$R_{\beta}$, 评论的helpful votes为$R_{h}$，则unhelpful votes($R_{u}$)为total votes减去helpful votes，净有用投票数为（$R_{h}-R_{u}$)，我们用净投票数加1作为因子再乘上量化后的评论，表征评价的可信度。

我们采用对数形式使得离散的各参数值更平滑，信誉模型初步建立为：
\begin{equation}
	F_{repu} =  log_2(R_{\alpha}+1)+ \beta \times log_2(R_{\beta}\times(R_{h}-R_{u}+1)+1)
	\label{priA}
\end{equation}

为了方便处理，在模型中，当净有用投票数为负数时，统一设为-1，即这种评论对声誉没有贡献。更进一步，超参数$\beta$一方面要能体现评级和评论的权重关系，另一方面还要能表征评论的可信度，而vine或者verified\_purchas字段为'Y'的评论是体验过产品的评论，更具有参考性。因此$\beta$的表达式如下：
\begin{equation}
...
\end{itemize}
然后我们对6种样本计算了这5天内的平均声誉值，结果如图：

%然后我们又统计了整个2015年8月，pacifier这6种样本的有效评论数(vine或verified purchase为yes)，近似表征该月的销量：
%\begin{figure}[H] 
%	\centering 
%	\includegraphics[width=0.6\textwidth]{figures/valid_review.png} 
%	\caption{The number and proportion of valid reviews of six samples in August 2015}
%	\label{valid_review} %用于文内引用的标签
%	
%\end{figure}
图中说明，2015年7月-8月pacifier中的xxxx产品声誉值最高，意味着更好的销售前景，xxx产品声誉值最低，意味着销售前景并不明朗。
